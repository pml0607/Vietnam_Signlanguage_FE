...
class LabeledVideoDataset(torch.utils.data.IterableDataset):
	...
	def _preprocess_path(self):
		...
		for video_path, label in self._labeled_videos:
			self.__labeled_videos_landmarks.append(((None, video_path), label))

		assert len(self.__labeled_videos_landmarks) == len(self._labeled_videos)
...
#region __next__
	def __next__(self) -> dict:
		...
		for i_try in range(self._MAX_CONSECUTIVE_FAILURES):
			...
			(_, video_path), info_dict = self.__labeled_videos_landmarks[video_index]
